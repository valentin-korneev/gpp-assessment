...
- `LVPCSS`;
- `IPSClient`;
- `BDA / Əməliyyat gününün idarə olunması modulu` - функциональный модуль подтвержден руководством пользователя от 10.03.2025; REST API и схемы `BDA`/`BDA_ARCHIVE` подтверждены технической спецификацией от 02.04.2025. Связь с `AdminConsole` и точная граница развертывания остаются в `Q-BDA-001`.
...
`SRC-BDA-001` и `SRC-BDA-002` вместе подтверждают функциональную и техническую область BDA на 02.04.2025. Модуль охватывает определения процессов и параметры, шаблоны, создание/закрытие/архивирование операционного дня, управление процессами дня, архив и сценарии мониторинга XÖHKS, платежей и файлов.

Техническая спецификация добавляет `IF-GPP-015`: административный REST API из 39 операций и физическое хранение в схемах `BDA`/`BDA_ARCHIVE`. Это повышает доказанность BDA от уровня пользовательского интерфейса до межслойной модели, но **не** доказывает наличие отдельного развертываемого серверного компонента или его отношение к `AdminConsole`. Точная граница клиентской и серверной частей, а также механизм запуска команд остаются в `Q-BDA-001`.

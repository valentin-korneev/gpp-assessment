...
Наличие экранов мониторинга не является утверждением о техническом владении соответствующими функциями: BDA отображает сведения по XÖHKS, платежам и файлам, но источник не доказывает, что именно BDA реализует расчеты, обработку платежей или доставку файлов (`INF-ARCH-008`). Связь с `AdminConsole`, границы серверной части и развертывания, а также сопоставление с REST остаются в `Q-BDA-001`; полный каталог процессов — в `Q-BDA-002`.

...
## Схема business process на странице 7

Страница 7 важна не только как подтверждение отдельных IAMAS/AVIS interfaces. Она задает **связный business-level flow** между четырьмя контурами: платежным интерфейсом, модулями HÖP, государственными информационными системами IAMAS/AVIS и сервисной организацией (XT).

Последовательность, которую можно утверждать непосредственно по схеме:

1. Плательщик выбирает организацию/услугу в third-party или HÖP payment interface.
2. Выбирается способ идентификации и вводится соответствующий параметр; после предварительной проверки запрос направляется в HÖP.
3. HÖP маршрутизирует identification либо в XT, либо в IAMAS/AVIS. XT идентифицирует плательщика по внутреннему коду/номеру документа; IAMAS/AVIS - по FIN/VÖEN.
4. Полученные payer data проверяются и регистрируются в HÖP.
5. HÖP запрашивает у XT debt information, сохраняет полученные сведения и возвращает их запрашивающей стороне вместе с payer data.
6. Плательщик может выбрать часть задолженностей и/или advance services; advance query направляется XT, который выполняет проверку и формирует ответ.
7. После получения информации о завершении платежа HÖP регистрирует payment и передает сведения XT; XT регистрирует payment information во внутренних системах.

Схема **не называет конкретные SOAP operations для каждого process box** и не раскрывает внутренний HÖP orchestrator/database component. Поэтому mapping этих шагов на `ServiceCompanyWS` operations используется только там, где он отдельно подтверждается contract tables/message sections, а не выводится из рисунка автоматически.

Canonical engineering representation сохранен в `02-as-is/diagrams/payer-service-payment-flow.mmd`.
